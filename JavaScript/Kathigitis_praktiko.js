document.addEventListener('DOMContentLoaded', function() {
    ...
    let thesisId = window.thesisId;
    console.log("Thesis ID:", thesisId);

    let formData = new FormData();
    formData.append("thesis", thesisId);

    fetch("http://localhost/Project-Web/Web%20Application%20Project/PHP/Kathigitis_praktiko.php", {
        method: "POST",
        body: formData,
    })
        ...
        function mapObject(obj) {
            return Object.fromEntries(
                Object.entries(obj).map(([key, value]) => [
                    key,
                    value === null ? "null" : (typeof value === "object" && value !== null ? mapObject(value) : value),
                ])
            );
        }

        // Transform the data object to replace all null values
        data = mapObject(data);
